    ...
    async function handleSetValue(e) {
        ...
        if (contract) {
            try {
                ...
                }, [name,address,variety,quantity,dateofharvest]);
                  const transaction = {
                    from: address,
                    to: proxyAddress,
                    data: data,
...
<NavLink to={`/Tracking2/${item.id}/${item.name}/${item.currentOwner}/${item.variety}/${item.quantity}/${item.dateofharvest}`} className="btn btn-primary" style={{ height: "fit-content" }}>Tracking</NavLink>
</td>
                                  </tr>
                                ))}
                                            </table>  
